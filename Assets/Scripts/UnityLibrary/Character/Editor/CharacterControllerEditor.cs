...
namespace Character
{
    ...
    public class CharacterEditor : Editor
    {
        ...
        private SerializedProperty mRightProp;
        ...
        private void DrawProperties()
        {
            ...
            if (mForwardProp.objectReferenceValue is Transform forward)
            {
                ...
                Handles.ArrowHandleCap(0, centerPosition + forwardVec * 0.5f, Quaternion.LookRotation(forwardVec), 0.5f,
                    EventType.Repaint);
                Handles.Label(forwardPosition, "Forward");
            }

            if (mRightProp.objectReferenceValue is Transform right)
            {
                Vector3 rightPosition = right.position;
                Vector3 rightVec = rightPosition - centerPosition;

                Handles.color = Handles.xAxisColor;
                Handles.ArrowHandleCap(0, centerPosition + rightVec * 0.5f, Quaternion.LookRotation(rightVec), 0.5f, EventType.Repaint);
            ...
            if (mUpProp.objectReferenceValue is Transform up)
            {
                ...
                Handles.ArrowHandleCap(0, centerPosition + upVec * 0.5f, Quaternion.LookRotation(upVec), 0.5f,
                    EventType.Repaint);
        ...
        private void UpdateProperties()
        {
            ...
            if (mFeetProp.objectReferenceValue is null)
            {
                ...
                if (feet is null)
                {
                    ...
                    feet.SetParent(body);
                    feet.localPosition = new Vector3(0f, -1f, 0f);
            ...
            if (mForwardProp.objectReferenceValue is null)
            {
                ...
                if (forward is null)
                {
                    ...
                    forward.localPosition = new Vector3(0f, 0f, 1f);
            ...
            if (mRightProp.objectReferenceValue is null)
            {
                Transform right = center.Find("Right");
                if (right is null)
                {
                    right = new GameObject("Right").transform;
                    right.SetParent(center);
                    right.localPosition = new Vector3(1f, 0f, 0f);
                    right.localRotation = Quaternion.identity;
                    right.localScale = Vector3.one;
                }

                mRightProp.objectReferenceValue = right;
            }
        ...
        private void InitProperties()
        {
            ...
            mRightProp = serializedObject.FindProperty("right");
